/*
  Code borrowed from https://github.com/cdharris/flutter_duration_picker/,
  with modification to allow the user to choose hourly and minute picks.
 */
...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
        centerPoint, radius * 0.88, new Paint()
      ..color = Theme
          .of(context)
          .canvasColor);
...
//            text: '${hours}${minutes > 0 ? minutes : ""}',
    ...
    TextPainter textMinPainter = new TextPainter(
        textAlign: TextAlign.center,
        text: new TextSpan(
            text: displayInHours ? "o'clock" : 'min.', //th: ${theta}',
            style: Theme
                .of(context)
                .textTheme
                .body1),
...
class _Dial extends StatefulWidget {
  ...
  @override
  _DialState createState() => new _DialState(this.displayInHours);
}

class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  bool displayInHours;

  _DialState(displayInHours) {
    this.displayInHours = displayInHours;
  }
...
//    _updateRotations(oldTheta, newTheta);
  ...
  List<TextPainter> _buildMinutes(TextTheme textTheme) {
    ...
    List<Duration> _clockMarkerValues;
    if (this.displayInHours) {
      _clockMarkerValues = <Duration>[
        const Duration(hours: 1, minutes: 0),
        const Duration(hours: 2, minutes: 0),
        const Duration(hours: 3, minutes: 0),
        const Duration(hours: 4, minutes: 0),
        const Duration(hours: 5, minutes: 0),
        const Duration(hours: 6, minutes: 0),
        const Duration(hours: 7, minutes: 0),
    ...
    <Duration>[
      ...
      const Duration(hours: 0, minutes: 42),
    ...
    for (Duration duration in _clockMarkerValues) {
    var painter = new TextPainter(
    text: new TextSpan(style: style, text: this.displayInHours ? duration.inHours.toString() : duration.inMinutes.toString()),
    textDirection: TextDirection.ltr,
    )..layout();
    labels.add(painter);
    }
...
class DurationPicker extends StatelessWidget {
  ...
  DurationPicker({this.duration = const Duration(minutes: 0),
